...
// TODO put this in `stack-config.js`
var stack_config = {
...
var using = {
		...
		line.beforeRender(function(data) {
			...
			each(data, function(d,i) {
				...
				if(isProperType && isProperAxis) {
					...
					if (d.values.length < stack_config.lineDotsThresholdSingle) {
		...
		bar.beforeRender(function(data) {
			...
			if (isolated_data.length > 0) {
				...
				if (this.x.$scale == "time" && parsed.data.length > 0) {
					...
					if (groupRangeBand > stack_config.maxColumnWidth) {
						bar.width(stack_config.maxColumnWidth);
						// each group width needs to be reduced if we are enforcing max width
						barGroupWidth -= (groupRangeBand - stack_config.maxColumnWidth);
					} else {
	...
	circles: function(circle, location) {
		...
		circle.beforeRender(function(data) {
			...
			var totalLineDotThresh = (totalLinePoints < stack_config.lineDotsThresholdTotal);
...
var cb_stack = d4.chart("cb-stack", function() {
	var singleLineDotThresh = false;
	var totalLinePoints = 0;

	var stackChart = d4.baseChart({
		config: {
			axes: {
				x: {
					scale: "ordinal",
		...
		.using("leftAxis", function(axis) {
			...
			axis.wrap(false);
			axis.scaleId("left");
			axis.orient(stack_config.axisOrient.left);
			axis.align(stack_config.axisOrient.left);
		})
		.using("rightAxis", function(axis) {
			axis.stagger(false);
			axis.wrap(false);
			axis.scaleId("right");
			axis.orient(stack_config.axisOrient.right);
			axis.align(stack_config.axisOrient.right);
		})
		.using("xAxis", function(axis) {
			...
			axis.afterRender(function(curAxis, data, chartArea, axisNode) {
				...
				coords[1] = coords[1] + stack_config.xAxisShift;
	...
	return d4.charts.stackedColumn();
});

module.exports = cb_stack;
